		...
		if (classname == null) {
			...
			try {
				...
				throw new IllegalArgumentException(
						"Component/Model not found '" + classname + "'");
			}
		}
	...
	private StringBuilder generateSource() throws Exception {
		if (control != null) {
			if (control.indexOf('.') == -1) {
				throw new IllegalArgumentException(
						"Not a valid control reference (object.field): '"
								+ control + "'");
			}
		}
		...
		for (KVP def : comps.entries) {
			...
			b.append(" public " + compClass + " " + def.getKey() + " = new "
					+ compClass + "();\n");
		}
		b.append("\n\n");
		...
		for (Param param : getParam()) {
			...
			b.append("  in2in(\"" + name[0] + '_' + name[1] + "\", " + name[0]
					+ ", \"" + name[1] + "\");\n");
		}
